    ...
    <div class="p-6">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-200">
                <i class="fas fa-calendar mr-2 text-blue-600"></i>{{ $meses[$month] }} {{ $year }}
            </h2>
            ...
            @for($d = 1; $d <= $diasEnMes; $d++)
                ...
                <a href="{{ request()->fullUrlWithQuery(['fecha' => $fecha]) }}"
                    class="relative p-2 text-sm rounded-xl transition-all duration-200 text-center
                        {{ $esHoy ? 'bg-blue-100 dark:bg-blue-900/50 font-bold' : 'hover:bg-gray-100 dark:hover:bg-gray-700/50' }}
                        {{ $tieneCitas ? 'bg-blue-50 dark:bg-blue-900/20' : '' }}">
                    <span class="{{ $esHoy ? 'text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300' }}">{{ $d }}</span>
                    @if($tieneCitas)
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                @foreach($citas as $i => $cita)
                    <tr class="hover:bg-blue-50/50 dark:hover:bg-blue-900/20 transition-colors duration-150 table-row-glow animate-fade-in stagger-{{ min($loop->index + 1, 10) }}">
                        <td class="px-6 py-4">
                            <div class="font-medium text-gray-900 dark:text-gray-100">{{ $cita->student->name }}</div>
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $cita->student->email }}</div>
                        </td>
...
@if($cita->estado === 'pendiente') bg-yellow-100 text-yellow-800 badge-pulse
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            @if($cita->estado === 'confirmada')
                                <form method="POST" action="{{ route('tutor.citas.completar', $cita) }}" class="inline">
                                    @csrf @method('PATCH')
                            ...
                            @if(in_array($cita->estado, ['pendiente', 'confirmada']))
                                <button onclick="showCancelModal({{ $cita->id }})" class="text-red-600 hover:text-red-900 mx-1 hover:scale-110 inline-block transition-transform" title="Cancelar">
...
function showCancelModal(id) {
    document.getElementById('cancelForm').action = '{{ url("tutor/citas") }}/' + id + '/cancelar';
