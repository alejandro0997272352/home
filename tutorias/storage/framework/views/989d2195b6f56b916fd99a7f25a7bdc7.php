...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    ...
    <a href="<?php echo e(route('estudiante.buscar')); ?>" class="btn-ripple inline-flex items-center px-4 py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5">
    ...
    <div class="p-6">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-200">
                <i class="fas fa-calendar mr-2 text-green-600"></i><?php echo e($meses[$month]); ?> <?php echo e($year); ?>
            ...
            <?php for($d = 1; $d <= $diasEnMes; $d++): ?>
                ...
                <a href="<?php echo e(request()->fullUrlWithQuery(['fecha' => $fecha])); ?>"
                    class="relative p-2 text-sm rounded-xl transition-all duration-200 text-center
                        <?php echo e($esHoy ? 'bg-green-100 dark:bg-green-900/50 font-bold' : 'hover:bg-gray-100 dark:hover:bg-gray-700/50'); ?>

                        <?php echo e($tieneCitas ? 'bg-green-50 dark:bg-green-900/20' : ''); ?>">
                    <span class="<?php echo e($esHoy ? 'text-green-700 dark:text-green-300' : 'text-gray-700 dark:text-gray-300'); ?>"><?php echo e($d); ?></span>
                    <?php if($tieneCitas): ?>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                <?php $__currentLoopData = $citas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $cita): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr class="hover:bg-green-50/50 dark:hover:bg-green-900/20 transition-colors duration-150 table-row-glow animate-fade-in stagger-<?php echo e(min($loop->index + 1, 10)); ?>">
                        <td class="px-6 py-4">
                            <div class="font-medium text-gray-900 dark:text-gray-100"><?php echo e($cita->tutor->name); ?></div>
                            <div class="text-sm text-gray-500 dark:text-gray-400"><?php echo e($cita->tutor->email); ?></div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-center">
                            <?php if(in_array($cita->estado, ['pendiente', 'confirmada'])): ?>
                                <button onclick="showCancelModal(<?php echo e($cita->id); ?>)" class="text-red-600 hover:text-red-900 hover:scale-110 inline-block transition-transform" title="Cancelar">
                            ...
                            <?php if($cita->estado === 'completada'): ?>
                                <?php if($cita->review): ?>
                                    <div class="flex items-center justify-center space-x-0.5">
                                        <?php for($s = 1; $s <= 5; $s++): ?>
                                            <i class="fas fa-star <?php echo e($s <= $cita->review->calificacion ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'); ?> text-xs"></i>
                                        <?php endfor; ?>
                                    </div>
                                <?php else: ?>
                                    <button onclick="showReviewModal(<?php echo e($cita->id); ?>, '<?php echo e($cita->tutor->name); ?>')" class="text-yellow-600 hover:text-yellow-800 hover:scale-110 inline-block transition-transform" title="Calificar">
...
function showCancelModal(id) {
    document.getElementById('cancelForm').action = '<?php echo e(url("estudiante/citas")); ?>/' + id + '/cancelar';
...
function showReviewModal(id, tutorName) {
    document.getElementById('reviewForm').action = '<?php echo e(url("estudiante/citas")); ?>/' + id + '/review';
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/alejandro/home/tutorias/resources/views/estudiante/citas.blade.php ENDPATH**/ ?>